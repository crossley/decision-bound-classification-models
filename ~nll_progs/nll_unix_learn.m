function negloglike = nll_unix_learn(in_params)

%  negloglike = nll_unix_learn(in_params)
%  returns the negative loglikelihood of the unidimensional X
%  - criterial learning variant - bound fit

%  Parameters:
%    params format:  [noise alpha_pos alpha_neg]
...
noise = in_params(1);
alpha_pos = in_params(2);
alpha_neg = in_params(3);

x = data(:,2);
fb = data(:,1) == data(:,4);

xc_init = x(1) + noise*randn;
